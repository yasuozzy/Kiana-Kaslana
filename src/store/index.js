...
const store = createStore({
  ...
  state(){
    return {
      ...
      // 新增 最近看过的商品
      recentViewed: JSON.parse( // ：JSON.parse是把字符串变成数组
        localStorage.getItem('recentlyViewed')|| '[]'), // 从浏览器本地拿数据 没有就是空数组
    }
  },
  ...
  actions: {
    /***** 登录Action - 处理用户登录逻辑 *****/
    // 登录动作 - {commit}是解构获取上下文对象中的commit方法，loginData是组件传入的数据
    login({commit,state}, loginData){
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          ...
          if(registeredUser && registeredUser.password === loginData.password){ //注册时存的密码 和 登录时输入的密码是否一致 但find返回的空数组直接不执行这些
            /***** 登录流程 - 登录成功处理 *****/
            // 当验证成功时 创建一个用户数据对象
            const userData1 = {
    ...
    register ({commit, state}, userdata) {
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(()=>{
          ...
          // /***** 注册流程 - 第4步：处理重复注册 *****/
          if(isExist){ // 如果邮箱已被注册（isExist为true）
